...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  4 08:56:06 2018
...
class MediumSpider(scrapy.Spider):
    ...
    start_urls = ["https://medium.com/topic/popular"]
...
#        claps_per_a = np.mean(claps)
    ...
    def parse(self,response):
        ...
        ordera = 0
        page = Selector(text = self.driver.page_source.encode('utf-8'))
        SET_SELECTOR = "div.u-borderBox"
        for med in page.css(SET_SELECTOR):
            # Sends article url to next scraper
            ordera +=1
            link = med.css('div.u-flex0 a::attr(href)').extract_first()
            print(link)
            yield scrapy.Request(med.css('div.u-flex0 a::attr(href)').extract_first(),
    ...
    def parse_recs(self,response): #parse each fan's other likes
        ...
        page = Selector(text = self.driver.page_source.encode('utf-8'))
        try:
            following =int(page.css("a[href*='following']::text").extract_first())
            print(following)
        except:
            following = 0
        try:
            followers = int(page.css("a[href*='followers']::text").extract_first())
            print(followers)
        except:
            followers = 0
        links = []
